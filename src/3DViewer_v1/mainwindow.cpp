...
void MainWindow::on_pB_openFile_clicked()  // отработка нажатия клавиши открытие файла
{
    ...
    if (filePath.isEmpty()) {
        ...
        if (error == 1) {
            ...
            ui->textEdit_fileInfo->setPlainText("File name: " + fileName + "\nFile Path: " + filePath);
...
void MainWindow::on_pB_screenShot_clicked()
{
    ...
    if (!pixmap.save(fileName)) {
        QMessageBox::warning(this, tr("Error"), tr("Screenshot didnt save, saved to \"%1\".")
...
void MainWindow::on_pB_GIF_clicked()
{
    ...
    QString filter = "*.gif";
    file_name = "/0";
    file_name = QFileDialog::getSaveFileName(this, "Save Gif?", QDir::homePath(), filter);
